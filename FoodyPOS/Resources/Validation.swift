...
struct Regex {
    ///Regular expression for user name allowing small and capital alphabets and length between 3 and 30.
    ...
    ///Regular expression for password allowing length between 8 and 15.
    static let passwordRegex   = "^.{8,15}$"//"^(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,15}$"
    ///Regular expression for email
    static let emailRegex      = "[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}"
     ///Regular expression for email and phone no.
